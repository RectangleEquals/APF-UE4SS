...
from ..controllers.controller import DiagnosticsController
...
class DiagnosticsPanel(PluginPanel):
    def __init__(self, host, **kwargs):
        ...
        self._host = host
        self._ctrl = DiagnosticsController(host)
    ...
    def _on_validate(self) -> None:
        ...
        items = self._ctrl.run_validation(self._detection)
        icon_map = {"ok": "✓", "warn": "⚠", "error": "✗"}
        result_lines: list[str] = []
        for item in items:
            self._results_layout.add_widget(
                _ResultRow(label=item.label, detail=item.detail, status=item.status)
            )
            line = f"{icon_map.get(item.status, '?')} {item.label}"
            if item.detail:
                line += f"  — {item.detail}"
