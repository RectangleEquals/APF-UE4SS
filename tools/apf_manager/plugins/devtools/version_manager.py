...
def commit_and_tag(component: str, version: str) -> tuple[bool, str]:
    """
    Stage the component's source file, commit with a standard message, push,
    create a namespaced tag, and push the tag.
    ...
    for cmd, label in steps:
        try:
            subprocess.check_output(cmd, cwd=str(_REPO_ROOT), stderr=subprocess.STDOUT, text=True)
