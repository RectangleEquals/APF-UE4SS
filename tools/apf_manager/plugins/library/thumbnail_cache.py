...
Usage:
    ...
    cache.get(app_id, on_loaded=lambda path: update_ui(path))
"""
...
class ThumbnailCache:
    ...
    def get(
        ...
        """
        Return the cached thumbnail path immediately, or start an async fetch.

        If the file is already cached, returns the path directly.
        If not cached, starts a background download and calls on_loaded(path)
        when done (on_loaded may be called with None on failure).
