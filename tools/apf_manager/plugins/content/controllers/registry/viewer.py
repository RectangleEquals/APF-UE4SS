...
_SPA_PATH = Path(__file__).parent.parent.parent / "views" / "assets" / "registry" / "viewer_spa.html"
...
def _folder_tree_to_spa_nodes(tree: "FolderTreeNode") -> list[dict]:
    ...
    def _convert(node: "FolderTreeNode", depth: int) -> dict:
        ...
        elif spa_type == "non_ap_mod":
            # X-5-C: handle node.mod is None (e.g. manifest parse failed → no DiscoveredMod)
            if node.mod:
                m = node.mod
                ...
                for n in node_list:
                    ...
                    if ntype == "mod" and n.get("mod_id"):
                        ...
                        if folder:
                            matched = next(
                                (m for m in mods
                                 if (m.folder == folder or (m.folder and m.folder.endswith("/" + folder)))
                                 and not m.mod_id),
                                None,
                            )
                            if matched:
