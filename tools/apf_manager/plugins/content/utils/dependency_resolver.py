"""
dependency_resolver.py — typed dependency graph utilities.
...
def resolve_install_order(
    ...
    folder_to_mod = {getattr(m, "folder_name", ""): m for m in ap_mods}
...
def resolve_uninstall_cascade(
    ...
    Note: only records with `capabilities_includes` or framework-mod dependency
    can be detected from InstallRecord alone. Store `dependencies` in InstallRecord
    in a future pass to make this fully general.
    """
