...
from ..chrome.content_toolbar import ContentToolbarWidget
from ..panels.templates_panel import TemplatesSectionMixin
from ..panels.mods_panel import ModsSectionMixin
...
class ContentTab(TemplatesSectionMixin, ModsSectionMixin, MDBoxLayout):
    ...
    def __init__(self, host, on_queue: Optional[Callable] = None, **kwargs):
        ...
        from ...controllers.tabs.content.controller import ContentController
        self._ctrl = ContentController(host)
        self._build_ui()
    ...
    def _do_refresh(self) -> None:
        ...
        # --- Templates section ---
        templates = self._ctrl.get_templates(self._game_id)
        ...
        # --- Mods section ---
        mods = self._ctrl.get_mods(self._game_id)
        self._all_mods = mods
    ...
    def _load_other_items_bg(self) -> None:
        ...
        def _bg():
            try:
                from ...models.descriptors.types import GithubReleaseBinary as _GRB, ManualBinary as _MB

                registry_ue4ss = self._ctrl.get_other_content(game_id)
                ...
                for entry in registry_ue4ss:
                    if isinstance(entry, _GRB) and entry.source and entry.source.repo.owner and entry.source.repo.repo and entry.source.tag:
                        fork_releases = self._ctrl.get_ue4ss_releases_for_content(
                ...
                ue4ss_official = self._ctrl.get_ue4ss_releases_for_content("UE4SS-RE", "RE-UE4SS")
                fw_items = self._ctrl.get_framework_releases_for_content()
    ...
    def _resolve_and_queue(self, mods: list, templates: list, other: list) -> None:
        ...
        queue_items, missing = self._ctrl.resolve_and_queue(mods, templates, other, self._game_id)
        ...
        o_mods      = [x for x, cat in queue_items if cat == "mod"]
        o_templates = [x for x, cat in queue_items if cat == "template"]
        o_other     = [x for x, cat in queue_items if cat == "other"]
