...
from ..chrome.badges import badge_text
...
class InstalledTab(MDBoxLayout):
    ...
    def _do_refresh(self) -> None:
        ...
        install_map: dict[str, "InstallRecord"] = self._ctrl.get_install_map(game_id)
        ...
        all_mods: list["ModInfo"] = self._ctrl.scan_mods()
        ...
        if not ue4ss_ok:
            ...
            mods_txt = self._ctrl.get_mods_txt()
    ...
    def get_installed_count(self) -> tuple[int, int]:
        try:
            return self._ctrl.get_installed_count()
        except Exception as exc:
