...
ROOT = Path(__file__).parent.parent  # tools/apf_manager/
...
try:
    _ver_ns: dict = {}
    exec((ROOT / "__version__.py").read_text(encoding="utf-8"), _ver_ns)
...
token_file = ROOT / "data" / ".github_token"
...
build_options = {
    ...
    "include_msvcr": True,
    "silent": True,
    "build_exe": str(ROOT / "build" / "APFManager"),
}
...
executables = [
    Executable(
        script=str(ROOT / "apf_manager_entry.py"),
        target_name="APFManager.exe",
        base=gui_base,
        icon=str(ROOT / "data" / "icon.ico") if (ROOT / "data" / "icon.ico").exists() else None,
    ),
    Executable(
        script=str(ROOT / "apf_manager_entry.py"),
...
def _gen_version_file() -> None:
    ...
    cmake_path = ROOT.parent.parent / "CMakeLists.txt"
    fw_ver = "?"
    try:
        m = re.search(
...
def _post_build():
    ...
    ps1_path = ROOT / "build" / "build_installer.ps1"
    ...
    ps1_path.write_text(
        ...
        '}\n'
        '\n'
        f'& $iscc /DMyAppVersion="{_MANAGER_VERSION}" "$PSScriptRoot\\..\\setup\\inno_setup.iss"\n',
        encoding="utf-8",
    )
