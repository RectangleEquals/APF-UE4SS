...
class APIPCServer::Impl
{
  ...
  private:
    ...
    void start_read(ClientConnection *conn)
    {
        ...
        BOOL success = ReadFile(conn->pipe, conn->read_buffer.data(), static_cast<DWORD>(conn->read_buffer.size()),
                                nullptr, &conn->overlapped);
    ...
    void process_received_data(ClientConnection *conn, DWORD bytes_received)
    {
        ...
        {
            APLogger::get()->log(LogLevel::Warn, "Incomplete message from " + conn->client_id);
            return;
        }
        ...
        {
            APLogger::get()->log(LogLevel::Error, "JSON parse error from " + conn->client_id + ": " + e.what());
        }
    }
...
class APIPCServer::Impl
{
  public:
    ...
    bool send_message(const std::string &, const IPCMessage &)
    {
        return false;
    }
    ...
    bool is_client_connected(const std::string &) const
    {
        return false;
    }
    size_t get_client_count() const
