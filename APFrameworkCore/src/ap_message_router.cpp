...
class APMessageRouter::Impl
{
  public:
                                                    ...
                                                    const std::string &sender_name)
    {
        if (!capabilities_)
        {
            APLogger::get()->log(LogLevel::Error, "Cannot route item - capabilities not set");
            return std::nullopt;
        }
    ...
    int64_t route_location_check(const std::string &mod_id, const std::string &location_name, int instance)
    {
        if (!capabilities_)
        {
            APLogger::get()->log(LogLevel::Error, "Cannot route location check - capabilities not set");
            return 0;
        }
    ...
    std::vector<int64_t> route_location_scouts(const std::string &mod_id,
                                               const std::vector<std::string> &location_names, bool create_hints)
    {
    ...
    void broadcast_lifecycle(LifecycleState state, const std::string &message)
    {
        ...
        APLogger::get()->log(LogLevel::Info, "Lifecycle -> " + lifecycle_state_to_string(state) +
