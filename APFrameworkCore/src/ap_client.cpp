...
class APClient::Impl
{
  public:
    ...
    bool connect(const std::string &server, int port, const std::string &game, const std::string &uuid)
    {
        if (client_)
        {
            disconnect();
    ...
    bool connect_slot(const std::string &slot_name, const std::string &password, int items_handling)
    {
        if (!client_)
        {
            return false;
    ...
    void send_location_scouts(const std::vector<int64_t> &location_ids, bool create_as_hint)
    {
        if (client_ && slot_connected_)
        {
            std::list<int64_t> ids_list(location_ids.begin(), location_ids.end());
    ...
    void send_bounce(const std::vector<std::string> &games, const std::vector<int> &slots,
...
bool APClient::connect(const std::string &server, int port, const std::string &game, const std::string &uuid)
{
    return impl_->connect(server, port, game, uuid);
}

bool APClient::connect_slot(const std::string &slot_name, const std::string &password, int items_handling)
{
...
void APClient::send_location_scouts(const std::vector<int64_t> &location_ids, bool create_as_hint)
{
...
void APClient::send_bounce(const std::vector<std::string> &games, const std::vector<int> &slots,
                           const std::vector<std::string> &tags, const nlohmann::json &data)
{
