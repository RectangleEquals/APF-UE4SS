...
static std::string build_goal_comment_block(const nlohmann::json &goals_arr)
{
    ...
    out += prefix + "If not set, completion falls back to: all accessible in-logic locations checked.\n";
    out += indent + "#\n";
    out += sep();
    ...
    out += sep();
    out += indent + "#\n";
    out += prefix + "To use a goal, uncomment and edit the line below (see table above):\n";
    out += indent + "#\n";
    out += prefix + "goal: " + goals_arr[0].value("name", "") + "\n";
    out += indent + "#\n";
    out += prefix + "If the goal is not set, undefined, or misspelled, completion falls\n";
    out += prefix + "back to the default: all accessible in-logic locations checked.\n";
    out += indent + "#\n";
    return out;
}
...
std::string APGeneratedConfig::serialize_to_yaml() const
{
    ...
    // Decode goals from capabilities for the comment block.
    ...
    // Emit YAML to string (top-level fields only — game section built manually below)
    std::string output;
    ...
    // Emit top-level fields (description, name, game, requires)
    ryml::emitrs_yaml(tree, &output, /* append */ true);

    // -------------------------------------------------------------------------
    // Build game section manually for full control over field order and comments
    // -------------------------------------------------------------------------
    const std::string indent = "  ";
    const std::string prefix = indent + "# ";

    std::string game = ap_world_name_ + ":\n";

    // 1. Goal comment block at top
    if (!goals_for_comment.empty())
    {
        game += build_goal_comment_block(goals_for_comment);
    }
    else
    {
        // No goals defined — explain the default "all locations" completion
        game += indent + "# " + std::string(70, '-') + "\n";
        game += prefix + "Goal Completion\n";
        game += prefix + "No goals are defined for this session.\n";
        game += prefix + "Completion condition: all accessible in-logic locations\n";
        game += prefix + "must be checked.\n";
        game += indent + "# " + std::string(70, '-') + "\n";
        game += indent + "#\n";
    }

    // 2. Mod options — description comment above each value
    // (Skip "goal" — it is handled exclusively inside the goal comment block above)
    for (const auto &def : option_definitions_)
    {
        if (def.key == "goal")
            continue;
        ...
        // Comment line: description + type hint
        if (!def.description.empty())
        {
            std::string detail;
            if (def.type == "range")
            {
                detail = " (Range: " + std::to_string(def.range_start)
                       + "-" + std::to_string(def.range_end) + ")";
            }
            else if (def.type == "text_choice" && !def.choices.empty())
            {
                detail = " (Options:";
                for (const auto &c : def.choices) detail += " " + c + ",";
                detail.back() = ')';
            }
            ...
            game += prefix + def.description + detail + "\n";
        }
        ...
        if (def.type == "toggle")
        {
            game += (value == "true" || value == "1") ? "true" : "false";
        }
        else if (def.type == "range")
        {
            try { game += std::to_string(std::stoi(value)); }
            catch (...) { game += value; }
        }
        else
        {
    ...
    // 3. capabilities_data — LAST, single-quoted (base64 contains no ' characters)
    game += prefix + "!! DO NOT MODIFY !! The value below contains encoded game data\n";
    game += prefix + "required by the framework. Editing it will break your connection.\n";
    game += indent + "capabilities_data: '" + capabilities_base64_ + "'\n";
