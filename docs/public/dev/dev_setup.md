...
This guide covers setting up a local build environment for the **AP Framework C++ core** and the **APF Manager** tool.

**Who this guide is for — pick your path:**

| Reader type | Sections needed |
|-------------|----------------|
| **Mod developer** — writing a UE4SS mod using APClientLib, no C++ build needed | Sections 6–8 (Python/Manager env + `__version__.py` + GitHub token) |
| **Source builder** — building the framework DLLs from source | All sections 1–8 (C++ toolchain + Python env) |
| **Contributor/collaborator** — submitting changes, managing releases | All sections + section 9 (versioning strategy) |
...
| Tool | Version | Notes |
|------|---------|-------|
| **Git** | Any | Required to clone vcpkg. Download from [git-scm.com](https://git-scm.com) |
...
> **Note:** Paths like `<tools_path>` below are examples — substitute your own preferred location (e.g. `C:\Dev`, `D:\Tools`, etc.).

```powershell
# Clone vcpkg somewhere permanent (e.g. <tools_path>\vcpkg)
git clone https://github.com/microsoft/vcpkg.git <tools_path>\vcpkg
cd <tools_path>\vcpkg
.\bootstrap-vcpkg.bat
...
Update `.vscode/settings.json` to point to your vcpkg install location:

```json
{
    "cmake.configureSettings": {
        "CMAKE_TOOLCHAIN_FILE": "<tools_path>\\vcpkg\\scripts\\buildsystems\\vcpkg.cmake"
    }
}
```
...
> **Note:** The `--query-driver` path above uses the **Community** edition install location. For **Enterprise** or **Professional**, replace `Community` with `Enterprise` or `Professional` in the path.

`compile_commands.json` is auto-generated by CMake + Ninja at the **project root** on first build (via the `cmake.copyCompileCommands` setting in step 3). Clangd reads it to map each source file to its exact compile flags, enabling accurate symbol resolution, go-to-definition, and diagnostics. After configure (step 3), clangd will index the project automatically.
