...
void APClientManager::handle_ipc_message_for_context(APClientContext *ctx, const ap::ClientIPCMessage &msg)
{
    ...
    if (msg.type == IPCMessageType::EXECUTE_ACTION)
    {
        ...
        int         exec_delivery_index = msg.payload.value("delivery_index",  -1);
        ctx->callbacks.invoke_item_received(item_id, item_name, sender, exec_location_id, exec_is_self,
                                            exec_handled_by, exec_delivery_index);
    }
    else if (msg.type == IPCMessageType::ITEM_RECEIVED)
    {
        ...
        ctx->callbacks.invoke_item_received(item_id, item_name, sender, location_id, is_self,
                                            handled_by, delivery_index);
    }
...
int APClientManager::create_lua_module_impl(lua_State *L, APClientContext *ctx)
{
    ...
    // item_handled(id_or_name [, silence [, delivery_index]])
    ...
    // silence=true + delivery_index: suppresses future on_item_received for THIS mod for that
    //   specific delivery only (persisted across reconnects via silenced_deliveries in session state).
    // silence=true without delivery_index: no-op for silencing (logs warn); handled_by still set.
    module["item_handled"] = [ctx](sol::object id_or_name, sol::optional<bool> silence_opt,
                                   sol::optional<int> delivery_index_opt) -> void
    {
        if (!ctx->ipc_client->is_connected()) return;
        bool silence        = silence_opt.value_or(false);
        int  delivery_index = delivery_index_opt.value_or(-1);
        ap::ClientIPCMessage msg;
