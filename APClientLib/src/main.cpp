...
static void notify_framework_of_error(const std::string &error_type, const std::string &details)
{
    ...
    msg.payload = {{"error_type", error_type}, {"details", details}, {"mod_id", g_mod_id}};
...
void handle_message(const ap::ClientIPCMessage &msg)
{
    ...
    invoke_optional_callback(g_callback_message, "on_message",
                             [&](sol::protected_function &cb) { return cb(msg.type, msg.payload.dump()); });
    ...
    if (msg.type == IPCMessageType::EXECUTE_ACTION)
    {
        ...
        if (g_action_executor)
        {
            ...
            invoke_optional_callback(g_callback_item_received, "on_item_received",
    ...
    else if (msg.type == IPCMessageType::LIFECYCLE)
    {
        ...
        invoke_optional_callback(g_callback_lifecycle, "on_lifecycle",
                                 [&](sol::protected_function &cb) { return cb(state, message); });

        // Specific lifecycle callbacks
        if (state == "ACTIVE")
        {
            invoke_optional_callback(g_callback_state_active, "on_state_active",
                                     [](sol::protected_function &cb) { return cb(); });
        }
        else if (state == "ERROR_STATE")
        {
            invoke_optional_callback(g_callback_state_error, "on_state_error",
                                     [&](sol::protected_function &cb) { return cb(message); });
        }
    }
    else if (msg.type == IPCMessageType::REGISTRATION_RESPONSE)
    {
        ...
        if (success)
        {
            invoke_optional_callback(g_callback_registration_success, "on_registration_success",
                                     [](sol::protected_function &cb) { return cb(); });
        }
        else
        {
            invoke_optional_callback(g_callback_registration_rejected, "on_registration_rejected",
    ...
    else if (msg.type == IPCMessageType::ERROR_MSG)
    {
        ...
        invoke_optional_callback(g_callback_error, "on_error",
                                 [&](sol::protected_function &cb) { return cb(code, error_message); });
    }
...
// Temporarily disable warning for "unreferenced local variable"
#pragma warning(push)
...
int create_lua_module(lua_State *L)
{
    ...
    g_ipc_client->set_connect_handler([]() {
        invoke_optional_callback(g_callback_connect, "on_connect", [](sol::protected_function &cb) { return cb(); });
    });

    g_ipc_client->set_disconnect_handler([]() {
        invoke_optional_callback(g_callback_disconnect, "on_disconnect",
                                 [](sol::protected_function &cb) { return cb(); });
    });
    ...
    module["is_connected"] = []() -> bool { return g_ipc_client && g_ipc_client->is_connected(); };

    // get_current_state() -> string (returns cached lifecycle state)
    module["get_current_state"] = []() -> std::string { return g_current_lifecycle_state; };
    ...
    module["register_mod"] = []() -> bool {
        ...
        msg.source = g_mod_id;
        msg.target = "framework";
        msg.payload = {{"mod_id", g_mod_id}, {"version", g_mod_version}};
    ...
    module["log"] = [](const std::string &level, const std::string &message) { log_internal(level, message); };
    ...
    module["on_lifecycle"] = [](sol::protected_function callback) { g_callback_lifecycle = callback; };

    module["on_message"] = [](sol::protected_function callback) { g_callback_message = callback; };

    module["on_error"] = [](sol::protected_function callback) { g_callback_error = callback; };

    // =========================================================================
    // Callback Registration - Specific
    // =========================================================================

    module["on_connect"] = [](sol::protected_function callback) { g_callback_connect = callback; };

    module["on_disconnect"] = [](sol::protected_function callback) { g_callback_disconnect = callback; };
    ...
    module["on_item_received"] = [](sol::protected_function callback) { g_callback_item_received = callback; };

    module["on_state_active"] = [](sol::protected_function callback) { g_callback_state_active = callback; };

    module["on_state_error"] = [](sol::protected_function callback) { g_callback_state_error = callback; };

    module["on_command_response"] = [](sol::protected_function callback) { g_callback_command_response = callback; };
